...
def convert_linked_to_coe(linked_memory: List[str], data_map: List[int], 
                         text_coe_path: str, data_coe_path: str) -> None:
    """
    ...
    text_coe_lines.extend(linked_memory)
    
    text_coe_content = '\n'.join(text_coe_lines)
    try:
        with open(text_coe_path, 'w', encoding='utf-8') as f:
            f.write(text_coe_content)
    except Exception as e:
    ...
    for i in range(0, len(data_map), 4):
        ...
        data_coe_lines.append(f"{word:08X}".lower())
    
    data_coe_content = '\n'.join(data_coe_lines)
    try:
        with open(data_coe_path, 'w', encoding='utf-8') as f:
            f.write(data_coe_content)
    except Exception as e:
