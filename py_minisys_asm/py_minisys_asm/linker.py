...
def _load_bios() -> List[str]:
    ...
    try:
        ...
        # 转换为十六进制指令
        bios_hex = []
        for instr in bios_program.text_seg.instructions:
            try:
                bios_hex.append(instr.to_hex(zero_x=False).lower())
            except Exception:
        ...
        # 限制在BIOS大小内
        return bios_hex[:BIOS_SIZE // 4]
    except Exception as e:
        # 如果无法加载真实BIOS，使用简化版本
        print(f"警告: 无法加载真实BIOS文件 {bios_file}: {str(e)}")
...
def _load_interrupt_handler() -> List[str]:
    ...
    try:
        ...
        print(f"警告: 无法加载真实中断处理程序文件: {str(e)}")
